...
// findmy_adv ready callback (runs once on the host task after sync): publishes
...
status_t ble_adv_init(tag_t *tag) {
    ...
    // findmy_adv is a standalone component returning esp_err_t; collapse it to
    // the firmware's status_t convention at this boundary.
    return findmy_adv_init(on_ready, CONFIG_ESPTAG_ADV_INTERVAL_MS) == ESP_OK
               ? STATUS_OK
               : STATUS_ERR;
}
...
static void on_ready(void) {
    // Publish the boot-epoch identity. findmy_adv advertises immediately since
    // the stack has synced by the time on_ready fires.
    if (findmy_adv_set_key(s_tag->p_curr) != ESP_OK) {
...
static void on_rotate(struct ble_npl_event *ev) {
    ...
    // findmy_adv_set_key stops and restarts advertising under the new key.
    if (findmy_adv_set_key(s_tag->p_curr) != ESP_OK) {
